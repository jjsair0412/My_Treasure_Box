...
kube-system replicaset.apps/dns-autoscaler-7df78bfcf 1 1 1 15h
...
```
### 2. 모든 node etcd stop
### 2.1 master etcd를 포함한 node etcd stop
...
```
### 2.2 master etcd를 포함한 모든 node /var/lib/etcd 폴더 제거
```
$ rm -rf /var/lib/etcd
```
## 3. master node 1번에서 작업 
### 3.1 restore
  - 복구 진행
```
...
- etcd 설정파일 수정
```
- ETCD_INITIAL_CLUSTER_STATE=new # 기존 existing 을 new로 변경 ( new로 설정하면 , 각 etcd의 토큰값을 새로 발급받게 된다. ) 
...
$ systemctl status etcd
```
### 3.4 정상적으로 실행됐는지 member list 명령어를 통해 확인
- 결과에 자기 자신만 출력되어야 한다.
...
$ systemctl status etcd
```
## 5. master node 1번에서 작업
### 5.4 add한 member 상태 start인지 확인
...
$ systemctl status etcd
```
## 7. 변경시켰던 etcd.env 파일 수정
- etcd.env 파일에서 자기 자신과 지금까지 add한 node만 바라보게끔 한 설정과 , INITAL 값을 existing으로 변경한다.
- 모든 노드에서 작업을 진행한다.
- 작업 순서는 무관하다.
```
- ETCD_INITIAL_CLUSTER_STATE=existing
...
# 변경 후 etcd.service 재시작
$ systemctl start etcd.env
```
## 8. 전체 node에서 작업
